#!/usr/bin/env python
"""Train MorphGSE — L3: full fine-tune, upsampling decoder (warmup + LLRD).
...
Usage (from project root):
    ...
    # → results/upsamp_warmup/pretrained/N10000/seed42/

    # random init + warmup + LLRD (контроль):
    uv run python scripts/upsamp_warmup/train.py \\
        --device cuda:0 --seed 42 --max-samples 10000
    # → results/upsamp_warmup/random/N10000/seed42/
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train MorphGSE: decoder warmup phase + LLRD fine-tune phase")
...
def main():
    ...
    model = MorphGSE(checkpoint_path=ckpt_path, frozen_backbone=False, device=str(device))
    ...
    best_val          = math.inf
    best_epoch        = 0
    epochs_since_best = 0  # counts from start of phase 2

    for epoch in range(args.warmup_epochs + 1, args.total_epochs + 1):
        phase2_epoch = epoch - args.warmup_epochs  # 1-indexed within phase 2
        ...
        # LR logging: always report n_blocks backbone LRs + decoder LR.
        if not args.no_llrd:
            # param_groups: [block_0, ..., block_{K-1}, decoder]
            lr_bb_blocks = [opt.param_groups[i]["lr"] for i in range(n_blocks)]
        else:
            # param_groups: [backbone, decoder] — report same LR for all blocks
