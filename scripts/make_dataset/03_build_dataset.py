#!/usr/bin/env python
"""Merge per-shot NPZ files into a single HDF5 dataset file.

Two-pass algorithm:
  Pass 1 — scan all NPZ to determine global max sizes (P_MAX, L_MAX, FC_MAX,
            FIL_MAX per coil).
  Pass 2 — pad variable-length arrays and concatenate into final arrays.
  Write  — chunked + gzip-compressed HDF5.
...
def _compute_stats(psi_init, pprime_map, ffprime_map, psi_target, splits_arr):
...
def main():
    ...
    print(f"Found {len(npz_paths)} NPZ files in {shots_dir}")

    # ── Pass 1: load all NPZ, determine max sizes ─────────────────────────────
    all_data: list[dict] = []
    ...
    for shot_idx, d in enumerate(all_data):
        ...
        for k in FEED_COILS:
            key = f"feed_{k}"
            all_feed[k].append(
                d[key].astype(np.float32) if key in d else np.full(T, np.nan, dtype=np.float32)
            )
    ...
    fil_r_arr = {c: np.stack(shot_fil[c]) for c in FIL_COILS}
    ...
    compress = {"compression": "gzip", "compression_opts": args.gzip} if args.gzip else {}
    chunk3d = (CHUNK, NX, NY)
    chunk1d = (min(1024, N),)
    ...
    with h5py.File(out_path, "w") as f:
        ...
        f.create_dataset("split",    data=splits_arr,  dtype=h5py.special_dtype(vlen=bytes))
        ...
        f.create_dataset("psi_norm",      data=psi_norm_arr,     chunks=(CHUNK, P_MAX), **compress)
        f.create_dataset("pprime_prof",   data=pprime_prof_arr,  chunks=(CHUNK, P_MAX), **compress)
        f.create_dataset("ffprime_prof",  data=ffprime_prof_arr, chunks=(CHUNK, P_MAX), **compress)
        f.create_dataset("prof_len",      data=prof_len_arr,     chunks=chunk1d)

        f.create_dataset("limiter_r",   data=limiter_r_arr,   chunks=(CHUNK, L_MAX), **compress)
        f.create_dataset("limiter_z",   data=limiter_z_arr,   chunks=(CHUNK, L_MAX), **compress)
        ...
        f.create_dataset("fcoil_c", data=fcoil_c_arr, chunks=(CHUNK, FC_MAX), **compress)
        ...
        for coil in FIL_COILS:
            ...
            fil_grp.create_dataset(f"{coil}_len",
    ...
    print(f"  target_mean={stats['target_mean']:.4e}  target_std={stats['target_std']:.4e}")
