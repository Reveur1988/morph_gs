#!/usr/bin/env python
"""Train MorphGSE — L1: frozen backbone, upsampling decoder.

Usage (from project root):
    # pretrained backbone (frozen) + upsampling decoder:
    ...
    # → results/upsamp_frozen/pretrained/N10000/seed42/

    # random backbone (frozen) + upsampling decoder:
    uv run python scripts/upsamp_frozen/train.py \\
        --device cuda:0 --seed 42 --max-samples 10000
    # → results/upsamp_frozen/random/N10000/seed42/
"""
import argparse
import json
import math
...
def parse_args():
    p = argparse.ArgumentParser(description="Train MorphGSE L1: frozen backbone, upsampling decoder")
    ...
    p.add_argument("--out-dir",      type=str,   default=None,
                   help="Output dir (default: results/upsamp_frozen/{pretrained|random}/N{N}/seed{seed}/)")
...
def main():
    ...
    print(f"[frozen/{pretrain}] decoder={n_dec:,}  backbone={n_bb:,}  "
